...
export function TSprite(aCanvas, aSpriteSheetImage, aSpriteAnimation, aDestination) {
  ...
  this.draw = function () {
    ...
    const x = spi.x + currentFrameIndex * spi.width;
    const y = spi.y;
  ...
  this.setIndex = function (aIndex) {
    currentFrameIndex = aIndex % spi.count;
  };
 ...
 * @param {FunctionStringCallback} - Call back function for when the button is clicked
 */
export function TSpriteButton(aCanvas, aSpriteSheetImage, aSpriteAnimation, aDestination, aClickCallBack) {
  const button = this;
  const scs = aCanvas;
  ...
  function spriteMouseUp(aEvent){
    onClicked.call(aEvent);
    if(button.disabled){
      sp.removeEventListener("mousemove", spriteMouseMove);
      sp.removeEventListener("mouseup", spriteMouseUp);
